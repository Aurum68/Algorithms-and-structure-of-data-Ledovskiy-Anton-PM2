...
public class MyBinaryTree {
    ...
    private String traversingStringByStack = "";
    private void traversingBinaryTreeByStack(Node node){
        Stack<Node> st = new Stack<>();
        st.push(node);
        while (!st.empty()) {
            Node buf_node = st.pop();
            traversingStringByStack += Integer.toString(buf_node.value) + " ";
            if (buf_node.right != null){
                st.push(buf_node.right);
            }
            if (buf_node.left != null) {
                st.push(buf_node.left);
            }
        }
    }
    ...
    public String getTraversingString(){ 
        ...
        traversingBinaryTreeByStack(this.root);
        return traversingStringByStack;
    }
}
